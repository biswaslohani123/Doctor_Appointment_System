...
const MyProfile = () => {
  ...
  return (
   <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-800 '>
        ...
        <p className='font-medium'>Phone:</p>
         ...
         <p className='font-medium'>Address:</p>
         {
          ...
          : <p className='text-gray-500'>{userData.address.line1} <br /><p>{userData.address.line2}</p></p>
         }
      </div>
    </div>
    <div >
    ...
    <div className='mt-10 '>
      {
        isEdit ? <button className=' cursor-pointer border border-blue-400 px-8 py-2 rounded-full hover:bg-blue-400 hover:text-white transition-all ' onClick={() => setIsEdit(false)}>Save Information</button> : <button className=' cursor-pointer border border-blue-400 px-8 py-2 rounded-full  hover:bg-blue-400 hover:text-white transition-all'  onClick={() => setIsEdit(true)}>Edit</button>
      }
    </div>
