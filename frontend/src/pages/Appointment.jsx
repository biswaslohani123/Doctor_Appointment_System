      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 ">
          ...
          <div className="flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0">
            ...
            <p className="text-gray-500 mt-2 font-medium">Appointment fee: <span className="text-gray-600">{currency}{docInfo.fees}</span></p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 w-full overflow-x-scroll mt-4">
          {
            docSlot.length && docSlot[slotIndex].map((item, index) => (
                <p onClick={() => setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-blue-400 text-white': 'text-gray-400  border border-gray-900'}`} key={index}>
        ...
        <button onClick={bookAppointment} className="bg-blue-400 text-white text-s  font-light px-14 py-3 rounded-full my-6 cursor-pointer">Book an appointment</button>
         </div>
